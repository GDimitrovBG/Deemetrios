...
**Текущо състояние на сървъра (от 17.09.2026):** Caddy е 2.6.2 (версията в
Ubuntu 24.04), а `file_server { status 404 }` иска ≥ 2.7. Затова е приложен
по-простият вариант — `/404.html` е махнат от `try_files`, `file_server`
връща естествения си 404:

```caddy
try_files {path} {path}/index.html      # без /404.html
```

Статусът вече е правилен; цената е, че при 404 се вижда стандартната
страница на Caddy вместо брандираната `dist/404.html`. Проверено след
промяната: всички 303 маршрута, вариантите с наклонена черта, `/en/*`,
`sitemap.xml`, `robots.txt` и `/api/health` отговарят 200 — махането на
catch-all fallback-а не е счупило нищо, защото всичко се prerender-ва.

**За да се върне брандираната страница** трябва Caddy ≥ 2.7 (официалното
repo на Caddy, `deb.caddyserver.com` — в Ubuntu apt има само 2.6.2). Тогава:
